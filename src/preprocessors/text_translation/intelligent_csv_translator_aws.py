...
class IntelligentCSVTranslatorAWS:
    """
    ...
    def __init__(self, aws_access_key_id: str = None, aws_secret_access_key: str = None, region_name: str = None, predetermined_languages: List[str] = None, use_llm: bool = False, llm_api_key: str = None, llm_base_url: str = None, llm_model: str = "gpt-5-nano"):
        """
        Initialize the translator with AWS credentials or LLM API.
        ...
        Args:
            ...
            use_llm: If True, use LLM for translation instead of AWS Translate
            llm_api_key: API key for LLM (required if use_llm=True)
            llm_base_url: Base URL for LLM API (required if use_llm=True)
        ...
        # Get non-translatable columns from the registry (needed for both LLM and AWS)
        ...
        # Patterns to identify non-translatable content (needed for both LLM and AWS)
        ...
        if self.use_llm:
            ...
            llm_base_url = llm_base_url or os.getenv("AZURE_OPENAI_API_BASE", "https://comexsoftllm.openai.azure.com/openai/v1/")
            
            if not llm_api_key:
                raise ValueError("LLM API key is required when use_llm=True. Set AZURE_OPENAI_API_KEY environment variable or pass llm_api_key parameter.")
            if not llm_base_url:
                raise ValueError("LLM base URL is required when use_llm=True. Set AZURE_OPENAI_API_BASE environment variable or pass llm_base_url parameter.")
            ...
            return  # Skip AWS Translate initialization
    ...
    def should_translate_column(self, series: pd.Series) -> Tuple[bool, str]:
        ...
        logging.info(f"Detected languages in sample: {languages}")
        if not languages:
...
def main():
    """Example usage of the IntelligentCSVTranslatorAWS"""
    
    # Get AWS credentials from environment or configuration
    if AWS_TRANSLATE_CONFIG:
        ...
        region_name = AWS_TRANSLATE_CONFIG.get("region", "us-east-1")
    else:
    ...
    # Initialize translator with predetermined languages for Spanish and English only
    translator = IntelligentCSVTranslatorAWS(
        aws_access_key_id=aws_access_key_id,
        aws_secret_access_key=aws_secret_access_key,
        region_name=region_name,
        predetermined_languages=['es', 'en']  # Limit detection to Spanish and English
    )
    
    # Example usage
    input_file = str(project_root) + "/data/processed/makro_mini.csv"
    output_file = str(project_root) + "/data/processed/makro_mini_translated_aws.csv"

    try:
        # Analyze the CSV first
        print("Analyzing CSV with predetermined languages (Spanish and English only)...")        
        # Translate the CSV - the predetermined languages will be used automatically
        print("\nStarting translation with multithreading...")
        results = translator.translate_csv(
            input_file, 
            output_file, 
            columns_with_language={'measuring_unit.format': 'es'},
            use_multithreading=True,  # Enable multithreading
            max_workers=20  # Use 5 concurrent threads
        )
