...
from src.api.api_utils.neo4j_queries import FilterTriplets, get_all_labels, get_nodes_by_label, generate_query_for_filter_triplets, verify_node_exists_query
...
@app.get("/labels")
async def get_all_labels_endpoint(limit: int = 100) -> Dict[str, Any]:
    """
    Get all existing node labels from Neo4j.
    ...
    try:
        ...
        query = get_all_labels(limit=limit)
        logging.info(query)
        ...
        if results:
            for record in results:
                # db.labels() devuelve { label: "Person" }
