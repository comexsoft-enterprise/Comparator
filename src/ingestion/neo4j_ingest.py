...
from src.ingestion.new_nodes_relationships import Neo4jNodesRelationshipsManager
...
class Neo4jCSVIngestor:
    ...
    def check_value_exists(self, label: str, property_name: str, value) -> bool:
        """
        ...
        try:
            ...
            with self.driver.session(database=NEO4J_DATABASE) as session:
                ...
                if existing_node:
                    logging.debug(f"Value '{value}' exists in label '{label}' property '{property_name}'")
                    return True
                else:
                    logging.debug(f"Value '{value}' does not exist in label '{label}' property '{property_name}'")
                    return False
                    
        except Exception as e:
            logging.error(f"Error checking if value exists in {label}.{property_name}: {e}")
            return False
