{
  ...
  outputs = { self, nixpkgs, nixpkgs-stable, devenv, ... } @ inputs:
  let
    system = "x86_64-linux";
    ...
    overlay-stable-numpy = final: prev: {
      python39Packages = prev.python39Packages // {
        ...
        numpy = (import nixpkgs-stable {
          inherit system;
          config.allowUnfree = true;
        }).python39Packages.numpy;

        # Use faiss from stable nixpkgs
        faiss = (import nixpkgs-stable {
          inherit system;
          config.allowUnfree = true;
    ...
    devShell.${system} = devenv.lib.mkShell {
      ...
      modules = [
        ({ pkgs, config, ...}:
        {
          ...
          env.AWS_PROFILE = builtins.getEnv "AWS_PROFILE";
          env.AWS_ACCOUNT_ID = builtins.getEnv "AWS_ACCOUNT_ID";
          env.AWS_REGION_NAME = builtins.getEnv "AWS_REGION_NAME";
          env.AWS_ACCESS_KEY_ID = builtins.getEnv "AWS_ACCESS_KEY_ID";
          env.AWS_SECRET_ACCESS_KEY = builtins.getEnv "AWS_SECRET_ACCESS_KEY";

          env.CRAWLEE_STORAGE_DIR = "/home/monk3yd/drive/comexsoft/report/src/storage";
