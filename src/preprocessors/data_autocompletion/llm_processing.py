...
class CSVCompleter:
    def __init__(
        ...
        Args:
            ...
            deployment_name: Azure OpenAI deployment name (e.g., "gpt-4.1_comexsoft")
            api_version: Azure OpenAI API version (e.g., "2024-02-15-preview")
            api_base: Azure OpenAI endpoint/base URL (e.g., "https://tu-recurso.openai.azure.com/")
    ...
    def complete_chunk(self, df_chunk: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            with open("output.txt", "a", encoding="utf-8") as file:
                file.write(f"[TOKENS] input={input_tokens} output={output_tokens} total={total_tokens} cached={cached_tokens} reasoning={reasoning_tokens}\n")
                file.write(f"[COST] input=${input_cost:.6f} cached=${cached_cost:.6f} output=${output_cost:.6f} total=${call_cost:.6f}\n")
                file.write(f"[ACCUMULATED] calls={self.call_count} total_cost=${self.total_cost:.4f}\n")
