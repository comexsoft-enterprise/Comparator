...
def prepare_for_neo4j_nodes(df: pd.DataFrame) -> pd.DataFrame:
    """
...
def process_dataframe_standards(df: pd.DataFrame, 
    ...
    result_df = prepare_for_neo4j_nodes(df)
    ...
    # Define el identificador por defecto
    default_id = "id"

    # Define los supermercados con identificadores específicos
    super_id_match = {
        "amazon": "asin"
    }

    # Función para obtener el identificador
    def get_super_id(supername: str) -> str:
        return super_id_match.get(supername, default_id)

    supername = os.path.splitext(os.path.basename(filename))[0]
    logging.debug(supername)

    # Default to 'id' if no specific match is found
    super_id = "id"
    for i, j in super_id_match.items():
        if i in supername:
            super_id = j
            break

    result_df['siid'] = supername + "-" + result_df[super_id].astype(str)
