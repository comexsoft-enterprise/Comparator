...
class Neo4jNodesRelationshipsManager:
    ...
    def process_products_batch(self, session, products_data: List[Dict[str, Any]]):
        ...
        """
        # Ensure we have an 'siid' field for the MERGE operation
        if 'siid' not in products_data:
            logging.error("Product data missing required 'siid' field")
            return
    ...
    def _prepare_nodes_batch(self, products_data: List[Dict[str, Any]]) -> Dict[NodeTypes, List[Dict[str, Any]]]:
        ...
        for product_data in products_data:
            product_name = self.get_value_case_insensitive(product_data, 'product_name')
            
            if self.is_empty_value(product_name):
                continue
            ...
            for node_type, source_column in self.node_source_columns.items():
                ...
                # Special handling for Offer
                if node_type == NodeTypes.OFFER:
                    if identifier_value in ['False', '0', 'false', False]:
                        continue
                    identifier_value = f"{product_name}_offer"
                ...
                if node_data not in nodes_by_type[node_type]:
                    nodes_by_type[node_type].append(node_data)
            
            # Country nodes (can come from multiple sources)
            ...
            if not self.is_empty_value(country):
                ...
                node_data = {'identifier': country, 'properties': properties}
                if node_data not in nodes_by_type[NodeTypes.COUNTRY]:
            ...
            if not self.is_empty_value(country_origin):
                properties = self._collect_node_properties(NodeTypes.COUNTRY, product_data)
                node_data = {'identifier': country_origin, 'properties': properties}
                if node_data not in nodes_by_type[NodeTypes.COUNTRY]:
                    nodes_by_type[NodeTypes.COUNTRY].append(node_data)
            ...
            if not self.is_empty_value(allergens_str):
                ...
                for allergen in allergens:
                    if not self.is_empty_value(allergen) and allergen.lower() != 'nan':
                        ...
                        if node_data not in nodes_by_type[NodeTypes.INGREDIENT]:
                ...
                if not self.is_empty_value(components_str):
                    ...
                    for component in components:
                        if not self.is_empty_value(component) and component.lower() != 'nan':
                            ...
                            if node_data not in nodes_by_type[NodeTypes.COMPONENT]:
    ...
    def _prepare_relationships_batch(self, products_data: List[Dict[str, Any]]) -> Dict[RelationshipTypes, List[Dict[str, Any]]]:
        ...
        for product_data in products_data:
            ...
            for rel_type, (source_node_type, target_node_type) in self.RELATIONSHIP_NODE_MAPPING.items():
                ...
                # Special handling for Offer
                if target_node_type == NodeTypes.OFFER:
                    offer_col = self.node_source_columns.get(NodeTypes.OFFER)
                    offer_value = self.get_value_case_insensitive(product_data, offer_col) if offer_col else None
                    if offer_value in ['False', '0', 'false', False, None]:
                        continue
                    target_value = f"{product_name}_offer"
    ...
    def _is_food_product(self, product_data: Dict[str, Any]) -> bool:
        ...
        if internal_type == InternalType.FOOD.value:
            return True
        
        category = self.get_value_case_insensitive(product_data, 'internalcategory')
        food_categories = [cat.value for cat in InternalCategoryFood]
        ...
        with self.driver.session() as session:
            for constraint in constraints:
                try:
                    ...
                    logging.info("Created constraint")
                except Exception as e:
