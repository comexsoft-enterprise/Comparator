...
RELATIONSHIP_NODE_MAPPING = {
        ...
        RelationshipTypes.HAS_INTERNAL_CATEGORY: (NodeTypes.INTERNAL_CATEGORY, NodeTypes.INTERNAL_TYPE),
        RelationshipTypes.HAS_INTERNAL_SUBCATEGORY: (NodeTypes.INTERNAL_SUBCATEGORY, NodeTypes.INTERNAL_CATEGORY),
        RelationshipTypes.COVERS: (NodeTypes.PRODUCT, NodeTypes.INTERNAL_SUBCATEGORY),
    }

NODE_IDENTIFIERS = {
        NodeTypes.PRODUCT: 'product_hash',  # MUST match the identifier used in nodes_relationships.py
...
class ColumnRegistry(BaseModel):
    ...
    def get_belongs_to_node_relationship_columns(self, node_relationship_type: NodeTypes) -> List[ColumnDefinition]:
        """Obtener columnas que pertenecen a un tipo específico de nodo o relación"""
        return [col for col in self.columns.values() if col.belongs_to == node_relationship_type]
