...
class VectorDataExtractor:
        ...
        for nutrient, config in unit_conversions.items():
            ...
            if value_col in df.columns and unit_col in df.columns:
                for idx, row in df.iterrows():
                    ...
                    # Skip if value or unit is null/empty
                    ...
                    if unit_lower in config['conversions']:
                        converted_value = value * config['conversions'][unit_lower]
                        df.at[idx, value_col] = converted_value
                        df.at[idx, unit_col] = config['standard_unit']
                    else:
                        # Log unknown unit
                        if unit_lower:
                            logger.warning(f"⚠️  Unknown unit '{unit}' for {nutrient}, keeping original")
        
        return df
    
    def calculate_price_without_vat(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        if 'vat' not in col_map:
            logger.info("⚠️  VAT column not found, skipping price_without_vat calculation")
            return df
        ...
        for source_pattern, target_pattern in price_pairs:
            source_col = col_map.get(source_pattern)
            target_col = col_map.get(target_pattern)
            
            # Only process if both source and target columns exist
            if source_col and target_col:
                # Calculate for rows where target is empty/null
                for idx, row in df.iterrows():
                    # Skip if target already has a value
                    if pd.notna(row[target_col]) and row[target_col] != '':
                        continue
                    
                    price = row[source_col]
                    vat = row[vat_col]
                    
                    # Skip if price or vat is null, empty, or 0
                    if pd.isna(price) or pd.isna(vat) or price == '' or vat == '' or price == 0 or vat == 0:
                        continue
                    
                    try:
                        # Convert to float
                        price_val = float(str(price).replace(',', '.').strip())
                        vat_val = abs(float(str(vat).replace(',', '.').strip()))
                        
                        # Skip if either is 0
                        if price_val == 0 or vat_val == 0:
                            continue
                        
                        # Calculate price without VAT: price / (1 + vat/100)
                        price_without_vat = price_val / (1 + vat_val / 100)
                        df.at[idx, target_col] = round(price_without_vat, 2)
                        
                    except (ValueError, TypeError, ZeroDivisionError):
                        continue
                
                logger.info(f"✅ Calculated {target_col} from {source_col}")
        
        # Process generic price columns
        for price_col in generic_price_cols:
            price_without_vat_col = price_col.lower().replace('price', 'price_without_vat')
            
            # Only process if the without_vat column already exists
            if price_without_vat_col not in col_map:
                continue
            
            target_col = col_map[price_without_vat_col]
            ...
            logger.info(f"✅ Calculated {target_col} from {price_col}")
    ...
    def process_input(self, input_path: str, drop_table: bool = False) -> int:
        ...
        # Normalize dataframe (standardize units)
        full_df = self.normalize_dataframe(full_df)
        
        # Calculate price_without_vat
        full_df = self.calculate_price_without_vat(full_df)
